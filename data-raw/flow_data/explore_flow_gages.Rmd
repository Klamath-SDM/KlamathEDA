...
tr_sf <- dataRetrieval::readNWISdv(11528700, "00060") |>  # 1965/10/01 - 2024/09/03
  select(date = Date, value =  X_00060_00003) |>  # rename to value
  as_tibble() |> 
  mutate(stream = "trinity river", # south fork
         order = 5,
         gage_agency = "USGS",
...
all_trinity_usgs_gages <- bind_rows(tr_sf, tr_hoopa, tr_ab_coffee, tr_lewiston, tr_douglas, tr_city, tr_helena, tr_brun) |>
  glimpse()
```
## Plot Trinity USGS Gages
...
leaflet(unique_gage_tr) |> 
  addTiles() |> 
    addCircleMarkers(
    popup = ~paste0("Stream name: ", stream, "<br>Gage Number: ", gage_number)
  )
```
