...
klamath_keno_or <- dataRetrieval::readNWISdv(11509500, "00010") |> 
  select(date = Date, value =  X_00010_00003) |>  # rename to value
  as_tibble() |>
  mutate(stream = "link river", # add additional columns for stream, gage info, and parameter
         gage_agency = "USGS",
         gage_number = "11509500",
         parameter = "temperature",
         statistic = "mean", # if query returns instantaneous data then report a min, mean, and max
...
mainsteam <- bind_rows(klamath_fl, klamath_keno, klamath_kl, klamath_or, klamath_keno_or)
...
other_temp_points <- bind_rows(sprague, williamsom, wood, fourmile_cl)
```
